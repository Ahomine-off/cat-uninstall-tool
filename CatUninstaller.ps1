...
# 🧩 Récupère les applis installées
$apps = @()
...
foreach ($a in $regApps) {
    $apps += [PSCustomObject]@{ Name = $a.DisplayName; Type = 'REG'; Cmd = $a.UninstallString }
}
$uwpApps = Get-AppxPackage | Where-Object { $_.Name -and $_.PackageFullName }
foreach ($u in $uwpApps) {
    $apps += [PSCustomObject]@{ Name = $u.Name; Type = 'UWP'; Cmd = $u.PackageFullName }
}
        ...
        if ($app) {
            try {
                if ($app.Type -eq 'REG') {
                    ...
                    Remove-AppxPackage -Package $app.Cmd -ErrorAction SilentlyContinue
                }
                [System.Windows.Forms.MessageBox]::Show("App '$sel' désinstallée ou tentative lancée.", "✅")
            } catch {
                [System.Windows.Forms.MessageBox]::Show("Échec désinstallation.", "❌")
            }
        }
    }
})

# 🧹 Bouton Retirer toutes les traces
...
$btnClean.Add_Click({
    ...
    if ($sel) {
        # 🔍 Scan des dossiers
        $paths = @("C:\Program Files", "C:\Program Files (x86)", "C:\ProgramData", "$env:APPDATA", "$env:LOCALAPPDATA", "$env:TEMP", "$env:USERPROFILE\AppData\Local\Packages")
        foreach ($root in $paths) {
            Get-ChildItem -Path $root -Directory -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.Name -like "*$sel*" -or $_.FullName -like "*$sel*" } | ForEach-Object {
